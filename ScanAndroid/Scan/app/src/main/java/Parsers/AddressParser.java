...
public class AddressParser extends PhoneParser {
    ...
    private String validateCity(String str) {
        ...
        for (String item : arrayOfPossibleCityName) {
            if (!item.isEmpty()) {
                if (Utils.cityList.contains(item) && notNameInCity(item)) {
                    return item;
                }
            }
        }
    ...
    private boolean notNameInCity(String name) {
        return Utils.cityList.contains(name) && !name.equals(this.name[0]);
    }
}